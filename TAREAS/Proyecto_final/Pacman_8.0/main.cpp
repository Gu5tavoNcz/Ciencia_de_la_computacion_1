...
template<class F>
void teclado_direcciones(Personaje *gustavo,vector<F*>);//F A,F B,F C,F D,F E,F G,F H,F I);
...
int main()
{
    ...
    Pacman* gustavo = Pacman::getInstance(pacman_imagen,pacman);
    ...
    fantasmas.push_back(&A);
    fantasmas.push_back(&B);
    fantasmas.push_back(&C);
    fantasmas.push_back(&D);
    fantasmas.push_back(&E);
    fantasmas.push_back(&G);
    fantasmas.push_back(&H);
    fantasmas.push_back(&I);

    play_midi(LEVEL_INTRO,0);
    teclado_direcciones(gustavo,fantasmas);
...
template<class F>
void teclado_direcciones(Personaje *gustavo,vector<F*> fant)//F A,F B,F C,F D,F E,F G,F H,F I);;
{
    while(!key[KEY_ESC])
    {
        if(d_anterior!=4)
